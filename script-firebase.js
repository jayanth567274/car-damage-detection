...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚗 Firebase Car Damage Detection App Loaded');
...
async function handleImageUpload(event) {
    ...
    try {
        ...
        // Add user ID to the result for Firebase storage
        const detectionData = {
            ...result,
        ...
        await saveDetectionToFirebase(detectionData);
...
/**
 * Save detection result to Firebase Firestore
 */
async function saveDetectionToFirebase(detectionData) {
    try {
        const docRef = await addDoc(collection(db, "detectionHistory"), {
            userId: currentUser.uid,
            damagedPart: detectionData.damagedPart,
            severity: detectionData.severity,
            estimatedCost: detectionData.estimatedCost,
            damageDescription: detectionData.damageDescription,
            damageLocation: detectionData.damageLocation,
            fileName: detectionData.fileName,
            timestamp: new Date()
        });
        
        console.log('Detection saved to Firebase with ID:', docRef.id);
        return docRef.id;
    } catch (error) {
...
/**
 * Load user detection history from Firebase
 */
async function loadHistory() {
    if (!currentUser) return;
    
    try {
        const q = query(collection(db, "detectionHistory"), where("userId", "==", currentUser.uid));
        ...
        console.log('Loaded', history.length, 'history records from Firebase');
    ...
    historyList.innerHTML = historyItems.map(item => `
        <div class="history-item">
            <div class="history-header">
                ...
                <div>
                    <span class="history-date">${new Date(item.timestamp.seconds * 1000).toLocaleDateString()}</span>
...
/**
 * Delete history item from Firebase
 */
async function deleteHistoryItem(id) {
    ...
    try {
        await deleteDoc(doc(db, "detectionHistory", id));
...
console.log('📝 Firebase-enabled script loaded successfully');